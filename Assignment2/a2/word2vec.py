...
def naiveSoftmaxLossAndGradient(
    ...
    loss1 = np.exp(np.dot(outsideVectors, centerWordVec))
    loss_denom = np.sum(loss1, axis = 0)
    loss = -np.dot(u_o, centerWordVec) + np.log(loss_denom)
    gradCenterVec = -u_o + np.sum(outsideVectors*loss1.reshape(-1,1), axis=0)/loss_denom
    gradOutsideVecs = np.vstack([centerWordVec]*outsideVectors.shape[0])*loss1.reshape(-1,1)/loss_denom
...
def negSamplingLossAndGradient(
    ...
    loss1 = sigmoid(np.dot(centerWordVec, outsideVectors[outsideWordIdx, :]))
    loss2 = sigmoid(-np.dot(negSampleVectors, centerWordVec))
    loss = -np.log(loss1) - np.sum(np.log(loss2))
    gradCenterVec = -outsideVectors[outsideWordIdx, :] * (1-loss1) + np.sum((1-loss2).reshape(-1,1)*negSampleVectors, axis=0)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for idx,i in enumerate(negSampleWordIndices):
        gradOutsideVecs[i, :] += centerWordVec * (1-loss2)[idx]
    gradOutsideVecs[outsideWordIdx, :] -= centerWordVec * (1-loss1)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        loss_w, gradCenterVec, gradOutsideVec = word2vecLossAndGradient(centerWordVectors[currentCenterWordIdx], word2Ind[outsideWord], outsideVectors, dataset)
        loss += loss_w
