...
def precompute_rotary_emb(dim, max_positions):
    ...
    for t in range(max_positions):
        for i in range(dim//2):
            ...
            rope_cache[t, i, :] = torch.tensor([math.cos(t*theta_i), math.sin(t*theta_i)])
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    rotated_x = None
    ### YOUR CODE HERE ###
    rotated_x = x.reshape(*x.shape[:-1], -1, 2)
    rotated_x = torch.view_as_complex(rotated_x)
    rope_cache = rope_cache[:x.shape[2],:,:]
    freq_cis = torch.view_as_complex(rope_cache)
    rotated_x = torch.view_as_real(rotated_x * freq_cis).flatten(3)
    ### END YOUR CODE ###
    return rotated_x
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.rope:
            ...
            rope_cache = precompute_rotary_emb(config.n_embd//config.n_head, max_positions)
            ### END YOUR CODE ###
