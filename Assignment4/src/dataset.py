...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunct_length = random.randint(4, max_length)
        trunct_document = document[:trunct_length]
        # 2
        masked_length = random.randint(1, max(trunct_length // 2, 1))  
        prefix_length = random.randrange(trunct_length - masked_length)
        prefix = trunct_document[:prefix_length]
        masked_content = trunct_document[prefix_length:prefix_length + masked_length]
        suffix = trunct_document[prefix_length + masked_length:]
        # 3
        pads_length = self.block_size + 1 - (len(prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content))
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR * pads_length
        # 4
        input_string = masked_string[:-1]
        output_string = masked_string[1:]
        # 5
        x = torch.tensor([self.stoi[char] for char in input_string if char in self.stoi], dtype=torch.long)
        y = torch.tensor([self.stoi[char] for char in output_string if char in self.stoi], dtype=torch.long)
        return x, y
        ### END YOUR CODE ###
